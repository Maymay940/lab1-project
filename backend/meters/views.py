...
def request_list(request):
    ...
    requests_list = (
        Request.objects.filter(user=current_user)
...
def request_detail(request, request_id):
    ...
    positions = ReadingPosition.objects.filter(request=request_obj).select_related(
        "water_meter"
    )
...
def add_reading(request):  # post /add-reading/ добавление показаний
    ...
    if request.method == "POST":
        ...
        draft_request = Request.objects.filter(
            status="draft", user=current_user
        ).first()
...
def api_meter_detail(request, meter_id):
    ...
    try:
        ...
        return JsonResponse({"error": "Счетчик не найден"}, status=404)
        ...
        result.append(
            {
                ...
                "total_consumption": (
                    float(req.total_consumption) if req.total_consumption else None
                ),
                "amount_to_pay": (
                    float(req.amount_to_pay) if req.amount_to_pay else None
                ),
            }
        )
...
def api_request_detail(request, request_id):
    ...
    try:
        ...
        data = {
            ...
            "completed_at": (
                request_obj.completed_at.strftime("%d.%m.%Y %H:%M")
                if request_obj.completed_at
                else None
            ),
            "positions": positions_data,
            "total_consumption": (
                float(request_obj.total_consumption)
                if request_obj.total_consumption
                else None
            ),
            "amount_to_pay": (
                float(request_obj.amount_to_pay) if request_obj.amount_to_pay else None
            ),
        }
        return JsonResponse(data)
...
def api_add_reading(request):
    ...
    try:
        ...
        existing = ReadingPosition.objects.filter(
            request=draft_request, water_meter=meter
        ).first()
        ...
        return JsonResponse({"success": True, "request_id": draft_request.id})
